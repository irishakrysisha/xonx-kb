...
NAPRYAMY = TAXONOMY_VALUES["Напрям"]     # напрям внутрішнього тренінгу
FORMATY = TAXONOMY_VALUES["Формат"]      # формат навчального матеріалу
...
# напрям внутрішнього тренінгу (про що «how it's made»)
_NAPRYAM_KEYWORDS = {
    "Структурування":  ["структурув", "structuring", "холдинг", "групова структур", "spv"],
    "Корпоративне":    ["корпоратив", "статут", "corporate governance", "збори", "board"],
    "Інвестиційне":    ["інвестиц", "safe", "раунд", "cap table", "фандрейз", "vc"],
    "Податкове":       ["податк", "tax", "оподатк", "пдв"],
    "Комплаєнс":       ["комплаєнс", "compliance", "kyc", "aml", "gdpr", "санкці"],
    "Процеси команди": ["процес команд", "онбординг", "onboarding", "внутрішн процес",
                        "регламент", "sop", "воркфлоу", "workflow", "як ми веде"],
    "Інструменти":     ["інструмент", "tool", "notion", "sheets", "шаблон процес",
                        "автоматизац", "checklist"],
    "Продажі-клієнти": ["продаж", "sales", "клієнт", "pitch", "перемовин", "лід", "crm"],
}
# формат навчального матеріалу
_FORMAT_KEYWORDS = {
    "Відео":         ["відео", "video", "запис екран", "screencast", "туторіал", "youtube"],
    "Запис зустрічі":["запис зустріч", "запис дзвінк", "meeting recording", "zoom", "воркшоп"],
    "Інструкція":    ["інструкц", "instruction", "how-to", "how to", "покроков", "manual"],
    "Чеклист":       ["чеклист", "checklist", "список перевір", "to-do", "todo"],
    "Презентація":   ["презентац", "presentation", "слайд", "deck", "pptx"],
    "Гайд":          ["гайд", "guide", "довідник", "playbook", "handbook", "методич"],
}
# ознаки внутрішнього тренінгу / «how it's made»
_TRAINING_SIGNALS = re.compile(
    r"як ми (це |)(робимо|веде|структур)|how it'?s made|навчальн|тренінг|training|"
    r"методолог|онбординг|onboarding|воркшоп|workshop|playbook|sop\b|регламент", re.IGNORECASE)
...
_TABLE_ROOTS = {"прецедент": "Прецеденти", "шаблон": "Шаблони", "templat": "Шаблони",
                ...
                "тренінг": "Внутрішні тренінги", "training": "Внутрішні тренінги",
                "навчальн": "Внутрішні тренінги", "how it": "Внутрішні тренінги"}
...
def _canon_table(t):
    t = (t or "").strip().lower()
    for tbl in ("Прецеденти", "Шаблони", "Рісьорчі", "Внутрішні тренінги", "Провайдери"):
        if t == tbl.lower():
            return tbl
...
def _heuristic(raw, hint_type=None):
    ...
    is_training = bool(_TRAINING_SIGNALS.search(low))

    if hint_type in ("Прецеденти", "Шаблони", "Рісьорчі", "Внутрішні тренінги",
                     "Провайдери"):
        ...
        table = "Внутрішні тренінги"; reasons.append("ознаки методології / «how it's made» → тренінг")
    elif is_qa:
    ...
    elif table == "Внутрішні тренінги":
        napr = _detect_one(low, _NAPRYAM_KEYWORDS) or "Процеси команди"
        fmt = _detect_one(low, _FORMAT_KEYWORDS) or "Гайд"
        fields.update({"Напрям": napr, "Формат": fmt})
        reasons.append(f"напрям: {napr}; формат: {fmt}")
    ...
    has_class = (_detect_cat(low) is not None
                 or _detect_one(low, _SPHERE_KEYWORDS) is not None
                 or _detect_one(low, _NAPRYAM_KEYWORDS) is not None)
    strong = sum([has_contact, has_ph, is_qa, is_training,
...
def _try_llm():
    ...
    def run(raw, hint_type=None):
        ...
        sys_prompt = (
            ...
            '{"table":"Прецеденти|Шаблони|Рісьорчі|Внутрішні тренінги|Провайдери",'
            '"fields":{...},'
            ...
            "• Рісьорчі — інтелектуальний продукт про ПРАВО: питання + аналіз + висновок "
            "(меморандум, Q&A, огляд практики). НЕ сам документ.\n"
            "• Внутрішні тренінги — «how it's made»: як МИ щось робимо всередині "
            "(методологія, онбординг, регламент/SOP, гайд, playbook, чеклист процесу, "
            "запис воркшопу). Про ВЛАСНІ процеси/інструменти команди, а не аналіз права.\n"
            ...
            "рісьорч vs тренінг — аналіз ПРАВА (рісьорч) vs як МИ це робимо (тренінг).\n\n"
            ...
            f"• Внутрішній тренінг: Напрям (про що) ∈ {NAPRYAMY} + Формат (вид "
            f"матеріалу) ∈ {FORMATY}. БЕЗ поля «Право».\n"
